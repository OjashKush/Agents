...
async def process_query(query: str):
    ...
    comparison_results = compare_rag_methods(query, standard_rag, reflective_agent)
    
    # Generate speech for standard_rag response
    try:
...
def streamlit_app():
    ...
    if st.button("Submit"):
        # Instead of making an HTTP request, call the query function directly
        result = rag_query({"question": question})
        
        # Display results (similar to your original code)
...
async def rag_query(query: Query):
    ...
    return await process_query(query.question)
...
def streamlit_app():
    ...
    if st.button("Submit"):
        with st.spinner("Processing..."):
            result = asyncio.run(process_query(question))
